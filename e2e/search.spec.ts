...
test("type filter pill browses Pokémon of that type", async ({ page }) => {
  ...
  await expect(
    page.getByRole("region", { name: "Search page" }).getByText("Pikachu", { exact: true }),
  ).toBeVisible();
});
...
test("switching type filter resets scroll to top", async ({ page }) => {
  ...
  await searchContent.evaluate((el: HTMLElement) => {
    el.scrollTop = 9999;
  });
...
test("evolution chain stage click navigates to that Pokémon", async ({ page }) => {
  ...
  await page.getByRole("option", { name: "Bulbasaur" }).click();
  await page
    .getByRole("region", { name: "Search content" })
    .getByRole("button", { name: "View Ivysaur" })
    .click();
...
test("evolution chain absent for Pokémon with no evolutions", async ({ page }) => {
  ...
  await expect(
    page.getByRole("region", { name: "Search content" }).getByText("EVOLUTION CHAIN"),
  ).not.toBeVisible();
});
...
test("EVOLVE button appears when party member can evolve", async ({ page }) => {
  ...
  await expect(
    page
      .getByRole("region", { name: "Search content" })
      .getByRole("button", { name: /EVOLVE Bulbasaur/ }),
  ).toBeVisible();
});
...
test("tapping a learner opens that Pokémon detail", async ({ page }) => {
  ...
  await page
    .getByRole("region", { name: "Search content" })
    .getByText("Charizard", { exact: true })
    .click();
