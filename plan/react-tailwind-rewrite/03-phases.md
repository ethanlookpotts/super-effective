...
- [x] Side-by-side comparison: vanilla (`origin/main`) vs refactor preview at 4 variants (mobile 390×844 + desktop 1280×720 × light + dark) — captured via custom Playwright script (`/tmp/parity-capture.mjs`) into paired screenshot dirs. Session 31 restored the major drifts (tabs→drawer, game-aware masthead, dotted background, page-header gradients + emoji titles, mixed-case rounded type pills, search gold-shell, party coverage ordering, settings copy + status badges, TMs inline scan button). Pre-existing differences remaining (dashed-vs-solid separators, HM chip strikethroughs on carrier card, sub-pixel spacing) noted as follow-ups in WORKLOG Session 31.
- [x] Manually exercise every feature path documented in `e2e/specs/*.md` — covered by all 98 `e2e/*.spec.ts` tests passing against `npm run preview` (Phase 8 + Phase 10 unparked the last 8 fixmes: desktop 3-col party grid via `md:grid-cols-3`; Send-to-PC from Search via new `📦 SEND TO PC` / `📦 IN PC BOX` button wired through `handleAddToPC`; sync conflict flow via moving `<ConflictModal>` out of `SettingsRoute` into the Shell so it's always reachable, plus `page.route`-based gist mocks driving `pull()` timestamp branches deterministically)
